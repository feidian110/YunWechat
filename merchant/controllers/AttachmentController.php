...
use addons\YunWechat\merchant\forms\PreviewForm;
...
class AttachmentController extends BaseController
{
    ...
    public function actionNewsEdit()
    {
        ...
        if ($request->isAjax) {
            ...
            try {
                ...
                Yii::$app->yunWechatService->attachment->editNews($attachment, $list, $isNewRecord);
                $transaction->commit();
        ...
        return $this->render('news-edit', [
            ...
            'list' => Yii::$app->yunWechatService->attachmentNews->formattingList($attach_id),
        ]);
    }
    ...
    public function actionCreate($type)
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->local_url) {
            try {
                $res = Yii::$app->yunWechatService->attachment->saveCreate($model);
    ...
    public function actionDelete($attach_id, $mediaType)
    {
        ...
        if ($model->delete()) {
            // 删除微信服务器数据
            $result = Yii::$app->yunWechatService->account->getAccount()->material->delete($model['media_id']);
    ...
    public function actionSync($type, $offset = 0, $count = 20)
    {
        // 查找素材
        try {
            $res = Yii::$app->yunWechatService->attachment->sync($type, $offset, $count);
            if (is_array($res)) {
