...
class MsgHistoryService extends Service
{
    ...
    {
        $setting = Yii::$app->yunWechatService->setting->getByFieldName('history');
        // 记录历史
        ...
        if (!isset($setting['utilization_status']) || $setting['utilization_status'] == StatusEnum::ENABLED) {
            // 插入规则统计
            !empty($data['rule_id']) && Yii::$app->yunWechatService->ruleStat->set($data['rule_id']);

            // 插入关键字统计
            if (!empty($data['keyword_id']) && !empty($data['rule_id'])) {
                Yii::$app->yunWechatService->ruleKeywordStat->set($data['rule_id'], $data['keyword_id']);
            }
        }
