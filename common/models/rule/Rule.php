...
use addons\YunWechat\common\models\base\AttachmentNews;
use common\behaviors\MerchantBehavior;
use Yii;

/**
 * This is the model class for table "yun_net_wechat_rule".
 *
 * @property int $id
...
class Rule extends \common\models\base\BaseModel
{
    ...
    {
        return $this->hasMany(Keyword::class, ['rule_id' => 'id'])->orderBy('type asc');
    }
    ...
    public function afterDelete()
    {
        $id = $this->id;
        // 关键字删除
        Keyword::deleteAll(['rule_id' => $id]);
        // 规则统计
        Stat::deleteAll(['rule_id' => $id]);
        // 关键字规则统计
        KeywordStat::deleteAll(['rule_id' => $id]);
    ...
    {
        // 更新状态和排序
        Keyword::updateAll(['sort' => $this->sort, 'status' => $this->status], ['rule_id' => $this->id]);
